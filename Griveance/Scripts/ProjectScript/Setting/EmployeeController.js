...
function CRUDController($scope, $filter, NgTableParams, Service) {
    ...
    function GetList() {
        Service.Get("Employee/GetList").then(function (rd) {
            if (rd.data.result !== 0) {
    ...
    $scope.AddUpdate = function () {
        if (!$scope.Selected) {
            if (form.valid()) {
                Service.Post("Employee/Add", { objModel: $scope.Model }).then(function (rd) {
                    if (rd.data.result === 1) {
                        $scope.Clear();
                        GetList();
            ...
            if (form.valid()) {
                Service.Post("Employee/Update", { objModel: $scope.Model }).then(function (rd) {
                    if (rd.data.result === 1) {
                        $scope.Clear();
                        GetList();
    ...
    $scope.Delete = function (item) {
        bootbox.confirm(recordConfirmDelete, function (result) {
            if (result) {
                Service.Post("Employee/Delete", { objModel: angular.copy(item) }).then(function (rd) {
                    if (rd.data.result === 1) {
                        GetList();
